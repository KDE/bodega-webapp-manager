...
var communicate = function(req, res, remoteRes, chunk) {
    ...
    if (req.session !== undefined && req.session.manager_authorized !== undefined) {
        req.session.manager_authorized = false;
    }
    ...
    if (reply.authStatus === true) {
        ...
        req.session.manager_authorized = true;
        req.session.servercookie = remoteRes.headers['set-cookie'];
        //res.redirect('index');

        http.get(partnersUrl(req, res), function(remoteRes) {
...
var requestUrl = function(req, res) {
    ...
    return utils.options(req, 'auth', query);
};

var partnersUrl = function(req, res) {
    //console.log(utils.options(req, 'partner/list'));
    return utils.options(req, 'partner/list', 0, true);
};
