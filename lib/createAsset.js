...
module.exports.show = function(req, res) {
    ...
    if (req.params.assetType == 'generic') {
        res.render('assetforms/createAsset', {
            network: app.config.network,
            inManagementGroup: req.session.partners[0].id == 0
        });
    } else if (req.params.assetType == 'book') {
        res.render('assetforms/createBookAsset', {
            network: app.config.network,
            inManagementGroup: req.session.partners[0].id == 0
        });
    } else if (req.params.assetType == 'plasmaPackage') {
        res.render('assetforms/createPlasmaPackageAsset', {
            network: app.config.network,
            inManagementGroup: req.session.partners[0].id == 0
        });
    } else if (req.params.assetType == 'obs') {
        res.render('assetforms/createObsAsset', {
            network: app.config.network,
            inManagementGroup: req.session.partners[0].id == 0
        });
    //mass create
    } else {
        res.render('assetforms/massCreateAsset', {
            network: app.config.network,
            inManagementGroup: req.session.partners[0].id == 0
        });
    }
