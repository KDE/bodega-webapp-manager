...
function doUpload(req, res, file) {
    var form = formstream();
    form.field( 'info', JSON.stringify({
                'name': req.body['assetName-' + file.name],
                'file': file.name,
                'basePrice': 0,
                'license': 1
                }));
...
module.exports.upload = function(req, res) {
    //FIXME: this seems a pretty shaky assumption about the structure of the req data
    //only one file specified
    if (req.files['fileselect'][0].path) {
        doUpload(req, res, req.files['fileselect'][0]);
    } else {
        for (var i in req.files['fileselect'][0]) {
            doUpload(req, res, req.files['fileselect'][0][i]);
        }
    }
}
